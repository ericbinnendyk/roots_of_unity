...
def root_sum(a0, dims, syms):
    if len(dims) > 1 and syms[1] == dims[1] - 1:
        a0 = remove_second_dim(a0, dims)
        dims = [dims[0]] + dims[2:]
        syms = [syms[0]] + syms[2:]
    ...
    if sym == rnum - 1:
        if len(dims) == 1:
            ...
            return root_sum(plus(a0[0], negate(a0[1])), dims[1:], syms[1:])

    if sym == 1:
        fact = 2
    else:
        facts = factorize(rnum // sym)
        fact = facts[-1]

    if fact == 2:
        a1 = rconj(a0, dims, power_cycle[rnum // (sym * fact)])
        s0 = plus(a0, a1)
        #print("s0 ==", s0)
        s0_x = root_sum(s0, dims, [sym * fact] + syms[1:])
        ...
        if mult == 0:
            ...
            #print("s1s ==", s1s)
            s1s_x = root_sum(s1s, dims, [sym * fact] + syms[1:])
        ...
        a = []
        for i in range(fact):
            a.append(rconj(a0, dims, power_cycle[rnum // (sym * fact) * i]))
        #print("a ==", a)
        s = []
        ss = []
        s.append(sumlist(a))
        ...
        s_x = []
        s_x.append(root_sum(s[0], dims, [sym * fact] + syms[1:]))
        ss_x = []
        ss_x.append(None)
        for i in range(1, fact):
            ss_x.append(root_sum(ss[i], [fact] + dims, [1] + [sym * fact] + syms[1:]))
